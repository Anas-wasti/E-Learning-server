...
exports.createOrder = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        const mailData = {
            order: {
                _id: (typeof course?._id === "string" ? course._id : course?._id?.toString())?.slice(0, 6) || "",
                name: course.name,
                price: course.price,
        ...
        if (user && course?._id) {
            const courseId = typeof course._id === "string" ? course._id : course._id.toString();
